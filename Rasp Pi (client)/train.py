...
MEAN_FILE1 = 'mean1.png'
POSITIVE_EIGENFACE_FILE1 = 'positive_eigenface1.png'
NEGATIVE_EIGENFACE_FILE1 = 'negative_eigenface1.png'
...
def train_classifier1():
	...
	neg_count = 0
	# Read all positive images
	for filename in walk_files(config.POSITIVE_DIR1, '*.pgm'):
		faces.append(prepare_image(filename))
		labels.append(config.POSITIVE_LABEL)
		pos_count += 1
	# Read all negative images
	for filename in walk_files(config.NEGATIVE_DIR1, '*.pgm'):
		faces.append(prepare_image(filename))
		labels.append(config.NEGATIVE_LABEL)
		neg_count += 1
	print 'Read', pos_count, 'positive images and', neg_count, 'negative images.'
	...
	# Save model results
	model.save(config.TRAINING_FILE1)
	print 'Training data saved to', config.TRAINING_FILE1

	# Save mean and eignface images which summarize the face recognition model.
	mean = model.getMat("mean").reshape(faces[0].shape)
	cv2.imwrite(MEAN_FILE1, normalize(mean, 0, 255, dtype=np.uint8))
	eigenvectors = model.getMat("eigenvectors")
	pos_eigenvector = eigenvectors[:,0].reshape(faces[0].shape)
	cv2.imwrite(POSITIVE_EIGENFACE_FILE1, normalize(pos_eigenvector, 0, 255, dtype=np.uint8))
	neg_eigenvector = eigenvectors[:,1].reshape(faces[0].shape)
	cv2.imwrite(NEGATIVE_EIGENFACE_FILE1, normalize(neg_eigenvector, 0, 255, dtype=np.uint8))
